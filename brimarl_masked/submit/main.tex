...
\vskip 0.2in
\begin{center}
\centerline{\includegraphics[width=\columnwidth]{learning_curve_bonus}}
...
\textbf{Improvements I would try next.} The bonus experiments already rule out two directions: switching to on-policy A2C, and re-designing the dense reward, neither of which addresses the causes above. What remains, in order of expected return: (1) more gradient updates per epoch ($2 \to 4$) with a lower learning rate ($5\cdot10^{-4}$) and a faster target refresh, since the curve was still drifting upward when $\varepsilon$ bottomed out; (2) Double DQN \cite{vanhasselt2016double} and a Huber loss to address (iii); (3) a \emph{terminal} win bonus (\texttt{win\_extra\_points}), the one reward change still untested, which targets the win/points mismatch directly rather than reshaping the dense signal; (4) hand-crafted state augmentation (e.g.\ explicit unseen-card features), which would transfer the baseline's key insight to the learned agent; (5) self-play after the best response converges, which the codebase already scaffolds.
